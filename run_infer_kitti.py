...
parser.add_argument('--output', metavar='DIR', default='./output_kitti', help='path to output folder')

parser.add_argument('--data_dir', metavar='DIR', default='./KITTI/training/image_2/',
...
voc_dir = './KITTI/training/'
args.test_list = os.path.join(voc_dir, 'train.txt')
...
def test(args, model, img_paths, save_path, spixeIds, idx, scale):
    ...
    # The orignal sz of nyu test set H*W
    ori_sz_spixel_map = F.interpolate(curr_spixl_map.type(torch.float), size=(int(H_), int(W_)), mode='nearest').type(
        torch.int)
    ...
    spixel_viz, spixel_label_map = get_spixel_image((ori_img + mean_values).clamp(0, 1), ori_sz_spixel_map.squeeze(),
                                                    n_spixels=1600 * scale * scale, b_enforce_connect=True)
...
def main():
    ...
    for scale in [0.6, 0.7, 0.8, 0.9, 1.0, 1.1, 1.2, 1.3, 1.4]:
        ...
        with open(args.test_list, 'r') as tf:
            ...
            for id in img_path:
                # print(id)
                img_path = os.path.join(data_dir, id.strip()+'.png')
                # print(img_path)
        ...
        for n in range(len(tst_lst)):
            ...
            mean_time += time
            count+=1
        print("avg_time per img: %.3f" % (mean_time / count))
