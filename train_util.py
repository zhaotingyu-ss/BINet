...
def label2gaussian_torch(labels, dis2bd, C=14):

    b, c, h, w = labels.shape
    line = np.arange(0, C * 10, 10).reshape(C, 1)
    xx = line * (np.ones((C, h * w)))

    x_ = np.stack((xx,) * b, axis=0)

    x_ = x_.reshape((b, C, h, w))
    labels_ = labels.data.cpu().numpy()

    dis2bd = dis2bd.data.cpu().numpy()
    norm_gt = np.sqrt(dis2bd[:,1, :, :] ** 2 + dis2bd[:,0, :, :] ** 2)

    sig = 1.2 -  np.exp(-(norm_gt**0.5))
    sig = np.stack((sig,) * C, axis=1)

    res = np.exp((- (x_ - labels_*10) ** 2) / (2 * sig ** 2)) / (math.sqrt(2 * math.pi) * sig)
    target = torch.from_numpy(res).cuda()
